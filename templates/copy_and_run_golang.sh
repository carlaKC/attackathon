...
if ! kubectl exec -it flagship -n warnet-armada -- bash -c 'cat run.sh | grep -q "go install"'; then
    ...
    kubectl exec -it flagship -n warnet-armada -- bash -c 'echo "export PEER=6" >> /run.sh'
    kubectl exec -it flagship -n warnet-armada -- bash -c 'echo "export RESOURCES=LIQUIDITY" >> /run.sh'
